...
- **E3.5 rate limiting (accepted):** PostgreSQL buckets + violations; `429 RATE_LIMIT_EXCEEDED`; E3.5d concurrency validation **green** — [`e3-5d-postgres-concurrency-validation.md`](../audits/e3-5d-postgres-concurrency-validation.md). Staging enablement allowed; production after staging review.
...
- **Business Context Builder (BCB):** isolated schema `business_context_builder` (migrations `0023`–`0024`); API routes under `/api/v1/business-context-builder/*`; Phase 3.1 AI next-question generation via `BusinessContextBuilderAiService` + `AiGatewayService` with static fallback (`BCB_AI_ENABLED`); API usage guide and curl examples in [`docs/api/business-context-builder.md`](../api/business-context-builder.md).

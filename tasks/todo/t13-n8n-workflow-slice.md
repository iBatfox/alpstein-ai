...
**Status:** T13.1–T13.5 **complete** (Gate 1 + Gate 2 passed); **stabilization required** before T14; T13.6/T13.7 deferred  
**Date:** 2026-05-24  
...
**Next:** [`post-t13-5-stabilization.md`](../../docs/ops/post-t13-5-stabilization.md) → **T14** customer ingress ([`t14-telegram-customer-ingress.md`](t14-telegram-customer-ingress.md)).

---

## Post–T13.5 stabilization (required before T14)

| ID | Task | Owner |
|----|------|-------|
| **STAB-1** | Commit T13.5 + docs (no secrets) | Human |
| **STAB-2** | Git tag `n8n-t13.5-gate2-*` | Human |
| **STAB-3** | Backup scrubbed workflow JSON | Human |
| **STAB-4** | Repo export = source of truth | Human |

Credential model for T14: [`telegram-channel-credentials.md`](../../docs/architecture/telegram-channel-credentials.md).
...
1. **STAB-1** — Commit + tag T13.5 Gate 2 baseline ([`post-t13-5-stabilization.md`](../../docs/ops/post-t13-5-stabilization.md)).  
2. **T14.1** — Telegram normalize mapping doc (then T14.2+ implementation per [`t14-telegram-customer-ingress.md`](t14-telegram-customer-ingress.md)).

**Not next:** T13.6 (retries) until after T14 customer path or explicitly re-prioritized.
